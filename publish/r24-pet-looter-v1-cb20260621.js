...
function panelHtml(){let p=petDef();return `<div class="petStatusCard"><div class="petOrbPreview"><img src="${esc(p.src)}" alt="${esc(p.name)}"></div><p><b>${esc(p.name)}</b> · 拾取宠物</p><p>普通和无尽模式自动拾取附近经验球与地上装备。大秘境不生效。</p><p><b>状态：</b>${hasPet()?'已拥有，进入普通/无尽自动出战':'未拥有'}</p><div class="petPickGrid">${petCards()}</div>${hasPet()?'':'<p><b>价格：</b>'+PET_CORE+' 魔核，购买后可从三只宠物中选择出战。</p>'}</div>`}
