...
function renderShop(){let costumes=SHOP_COSTUMES,items=[`<button class="shopCard ticket" data-buy-ticket><h2>秘境门票</h2><p>深渊秘境入场券</p><b>${SHOP_GOLD} 灵魂金币</b></button>`,`<div class="shopCard muted pet"><h2>宠物</h2><p>宠物柜台预留</p><b>即将开放</b></div>`];items=items.concat(costumes.length?costumes.map(c=>shopCostumeHtml(c)):[`<div class="shopCard muted"><h2>时装</h2><p>素材路径提供后上架</p><b>待添加</b></div>`]);let total=Math.max(36,Math.ceil(items.length/6)*6);while(items.length<total)items.push('<div class="shopSlot empty"><span>空</span></div>');$('shopSub').textContent=moneyText();$('shopBody').innerHTML=items.join('')}
